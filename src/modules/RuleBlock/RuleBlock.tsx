...
export const RuleBlock = ({ title, subtitle, options, index }: RuleBlockProps) => {
  ...
  return (
    <div
      className="w-full cursor-pointer rounded-3xl bg-zinc-700 bg-opacity-70 px-6 py-6"
      onClick={handleClick}
    >
      <div className="flex w-full items-center justify-between">
        <div className="flex flex-col space-y-2">
          <span className="text-text text-2xl font-bold">{`${index}. ${title}`}</span>
      ...
      <div
        ...
        className={`${showMore && 'mt-4'} text-text ml-4 space-y-2 overflow-hidden text-lg transition-all duration-300 ease-linear`}
      >
